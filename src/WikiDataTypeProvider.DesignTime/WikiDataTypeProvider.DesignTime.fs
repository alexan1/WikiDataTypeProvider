module WikiDataTypeProviderImplementation
...
type BasicErasingProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("WikiDataTypeProvider.DesignTime", "WikiDataTypeProvider.Runtime")], addDefaultProbingLocation=true)
...
type BasicGenerativeProvider (config : TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces (config, assemblyReplacementMap=[("WikiDataTypeProvider.DesignTime", "WikiDataTypeProvider.Runtime")])

    let ns = "WikiDataTypeProvider"
    let asm = Assembly.GetExecutingAssembly()
